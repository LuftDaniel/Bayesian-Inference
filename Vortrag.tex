...
Behandle Parameter $\theta$ wie Zufallsvariablen:\\
Statt Punktschätzer zu bilden, analysiere die Zufallsvariable mit Methoden der Wahrscheinlichkeitstheorie. Zum Beispiel Momente berechnen, Quantile etc.

\begin{center}
$\hat{\theta}_{Bayes} = \mathbb{E}_{\theta \vert D}[\theta]$
\end{center}
...
Sei $x= (x_1,\dots,x_n)$ ein Datenvektor generiert durch $X$. Weiterhin sei $\mathbb{P}(\theta)$ die a priori Verteilung der Parameter von $X$. Dann heißt
...
\begin{frame}
	Regressionsmodell der multiplen linearen Regression:\vspace{0,5cm}
...
\begin{frame}
	...
	\begin{align}
		\mathbb{P}(y|X,w,\lambda)\;=\;N(Xw,\sigma^2)
	\end{align}
	mit
	\begin{align}
		\mathbb{P}(\varepsilon)\;=\;N(0,\sigma^2)
	\end{align}
\end{frame}

\begin{frame}
	A Priori Annahme für Fehler $\varepsilon$ und Gewichte $w$ bei der Bayes Ridge Regression:
	\begin{align}
		\varepsilon\sim N(0, \sigma^2 I_n)\\[0,2cm]
		w\sim N(0, \tau^2 I_k)
	\end{align}
\end{frame}
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\draw[->,thick] (X) -- (M);
		\draw[->,thick] (M) -- (y);
		\draw[->,thick] (lambda) -- (eps) node[block,minimum width=1cm, midway, fill=white] {$\varepsilon\sim N(0,\sigma^2 I_n)$};
		\draw[red,->,thick] (alpha) -- (w) node[block,minimum width=1cm, midway, fill=white] {$w\sim N(0,\tau^2 I_k)$};
	\end{tikzpicture}
\end{frame}

\begin{frame}
	\begin{align}
		\hat{w}\;=\;\arg\underset{w}{\min}\sum_{i=1}^{n}\left(y-Xw\right)^2
	\end{align}
...
\begin{frame}
	Annahme: $w\sim Laplace(0, \tau)$ mit der Dichte
	\begin{align}
		f_{Laplace}(x|\mu,\sigma)\;=\;\frac{1}{2\sigma}\exp\left(-\frac{\left|x-\mu\right|}{\sigma}\right)
	\end{align}
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\draw[->,thick] (M) -- (y);
		\draw[->,thick] (lambda) -- (eps) node[block,minimum width=1cm, midway, fill=white] {$\varepsilon\sim N(0,\sigma^2 I_n)$};
		\draw[red,->,thick] (alpha) -- (w) node[block,minimum width=1cm, midway, fill=white] {$w\sim Laplace(0,\tau I_k)$};
	\end{tikzpicture}
\end{frame}
...
\begin{frame}
	\begin{tikzpicture}
		...
		\node [sum] (X) at (2,0) {$X$}; 
		\node [sum] (w) at (4,0) {$w$};
		\node [block, fill=white] (M) at (2, -1.5) {$y = sign(w^T x + b)$};
		%\node [sum, red] (lambda) at (0,2) {$\sigma$};
		%\node [red, sum] (alpha) at (4,2) {$\alpha$};
...
Bayes Framework: Nehme an $w, b$ sind Zufallsvariablen mit a priori Verteilungen!

Trainiere Posterior mit Daten $D$:
...
Bilde MAP Schätzer $\hat{w}_{MAP}, \hat{b}_{MAP}$ oder verwende Posterior weiter.
...
Nehme an, dass:   $w \sim \mathcal{N}(0,\frac{1}{\mu}), b \sim \mathcal{N}(0,\sigma^2_b)$ stochastisch unabhängig mit $\sigma^2_b \rightarrow \infty$.
...
Nehme weiterhin normalverteilte Fehler $e_i$ und unabhängige identische Verteilung bei den Daten $D$ an und erhalte
...
mit dem Fehler $e_i = 1 - y_i(w^T \phi(x_i) +b)$.
...
%\colorbox{red}{(NOTATION???) + MULTILAYER PERCEPTRONS FÜHREN} \\ 
%\colorbox{red}{NORMALERWEISE ZU NICHTKONVEXEN PROBLEMEN}\\
%\colorbox{red}{+ indefinite kernel paper}
\end{frame}
% d.h. man kann damit auch traditionelle Bayes rechnungen geschlossen anstellen, learning ist nicht so schwer
...
\begin{frame}
	...
	\begin{block}{LS-SVM-Problem}
		\begin{align}
			\min \mathcal{J}(w,e)\;=\;&\min\frac{1}{2}w^Tw+\frac{\gamma}{2}\sum_{i=1}^{n}e_i^2\\
...
\begin{frame}
	...
	\begin{align}
		\mathcal{L}(w,b,e;\alpha)&\;=\;\mathcal{J}(w,e)-\sum_{i=1}^{n}\alpha_i\left(y_i-\left(w^T\varphi(x_i)+b\right)-e_i\right)\\
...
%\colorbox{red}{FRAGWÜRDIG: SOLLEN WIR DAS ERWÄHNEN?}
...
\begin{frame}
	Gegeben sei ein Datensatz $D$ und ein Kern $K$. Wie findet man geeignete Hyperparameter $\mu, \xi$, welche den Grad der Regularisierung steuern?
	\begin{itemize}
...
Gull (1988) verwendete folgende a priori Verteilungen:

$\log(\mu) \sim \mathcal{N}(0, \sigma_\mu^2), \log(\xi) \sim \mathcal{N}(0, \sigma_\xi^2)$ stochastisch unabhängig, mit $\sigma_\mu^2, \sigma_\xi^2 \rightarrow \infty$.
...
\begin{align}
(\mu_{MAP}, \xi_{MAP}) = \underset{\mu, \xi}{\arg \min} & \hspace{0.2cm}\mathcal{J}(w_{MAP}, b_{MAP}) + \frac{1}{2}\sum_{i=1}^{N_{eff}}\log(\mu + \xi \lambda_i) \\
...
Seien $\mathcal{M}_1,...,\mathcal{M}_M$ Modelle, seien $D$ ein zugehöriger Datenvektor.
...
%\colorbox{red}{erkläre, dass das gleich P(M | D) ist bei uniformem prior + ist nicht likelihood ratio}
...
\begin{align}
\mathbb{P}(D \vert K_i) & \propto \mathbb{P}(D \vert \log(\mu_{i_{MAP}}), \log(\xi_{i_{MAP}}), K_i)\frac{\sigma_{\mu_i\vert D} \sigma_{\xi_i\vert D}}{\sigma_{\mu_i} \sigma_{\xi_i}}\\
&\propto \sqrt{\frac{\mu^{N_{eff}}\xi^{N-1}}{(\gamma_{eff}-1)(N - \gamma_{eff}) \prod_{j=1}^{N_{eff}}(\mu_{i_{MAP}} + \xi_{i_{MAP}}\lambda_j)}}
\end{align}
...
%ö, schreibe Hyperparameterproblem (level 2), ergänze für jeden Kernel muss man den durchgang, eigenwertproblem rausholen 
...
Da Logarithmus monoton ist, folgt
\begin{center}
$\hat{\theta}_{ML} = \arg\underset{\theta \in \Theta}{\max } \mathcal{L}(\theta)$
\end{center}

Falls latente Variablen im Modell, so können alle Messungen $x_i$ abhängig werden!

$\rightarrow $ Berechnung von $\hat{\theta}_{ML}$ schwieriger!
...
Nehme für latente Variable $z_i$ Verteilung $q_{z_i}$ an und verwende Jensens's Ungleichung!

\begin{center}

$\mathcal{L}(\theta) =  \overset{N}{\underset{i=1}{\sum}}\log(\int\mathbb{P}(x_i, z_i\vert \theta)dz_i)=  \overset{N}{\underset{i=1}{\sum}}\log(\int\frac{q_{z_i}(z_i)\mathbb{P}(x_i, z_i\vert \theta)}{q_{z_i}(z_i)}dz_i) \geq \overset{N}{\underset{i=1}{\sum}}\int q_{z_i}(z_i)\log(\frac{\mathbb{P}(x_i, z_i\vert \theta)}{q_{z_i}(z_i)})dz_i
= 
\overset{N}{\underset{i=1}{\sum}}\int q_{z_i}(z_i)\log(\mathbb{P}(x_i, z_i\vert \theta))dz_i - \int q_{z_i}(z_i) \log(q_{z_i}(z_i))dz_i =: \mathcal{F}(q_{z_1},...,q_{z_N},\theta)
$

\end{center}

%kommentar zu energie, entropie und freier energie Feynman
